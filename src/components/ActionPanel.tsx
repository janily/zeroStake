...
export function ActionPanel({
  ...
  return (
    <section className="rounded-[2rem] border border-ink/10 bg-white/78 p-5 shadow-diffusion backdrop-blur md:p-7">
      ...
      <div className="mt-7 grid grid-cols-1 gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <ActionBlock
          description="质押 Sepolia ETH 来参与 MetaNode 奖励分配。"
        ...
        <ActionBlock
          description="先提交解除质押请求，等待锁定区块结束后再提取。"
      ...
      {connected && !correctNetwork ? <p className="mt-4 text-sm text-rose-700">请切换到 Sepolia 测试网。</p> : null}
    </section>
  );
}
...
function ActionBlock({
  ...
  return (
    <div className="rounded-[1.5rem] border border-ink/10 bg-paper/80 p-4">
      ...
      <label className="mt-5 block text-sm font-medium text-ink/70" htmlFor={title}>
        {label}
      </label>
